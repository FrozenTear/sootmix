...
fn run_pipewire_loop(
    ...
    // Thread-local state
    let state = Rc::new(RefCell::new(PwThreadState::new()));
    let event_tx = Rc::new(event_tx);

    // Attach command receiver to main loop
    let main_loop_weak = main_loop.downgrade();
    let _cmd_receiver = cmd_rx.attach(main_loop.loop_(), {
        let state = state.clone();
        let event_tx = event_tx.clone();
        let core = core.clone();
        move |cmd| {
            handle_command(cmd, &state, &event_tx, &core, &main_loop_weak);
        }
    });
...
fn handle_command(
    ...
    match cmd {
        ...
        PwCommand::CreateVirtualSink { channel_id, name } => {
            ...
            // Virtual sink creation is handled via pw-loopback subprocess
            // See virtual_sink.rs for implementation
            match crate::audio::virtual_sink::create_virtual_sink(&name, &name) {
                Ok(node_id) => {
                    state.borrow_mut().virtual_sinks.insert(channel_id, node_id);
                    let _ = event_tx.send(PwEvent::VirtualSinkCreated { channel_id, node_id });
                }
                Err(e) => {
        ...
        PwCommand::SetVolume { node_id, volume } => {
            debug!("Setting volume on node {}: {}", node_id, volume);
